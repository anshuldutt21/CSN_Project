...
int main()
{
	...
	while(getline(fp,query))
	{
		...
		if(valid_array_l1[query_index]==1) // true if  line of l1 cache contains some data 
		{
			if(tag_array_of_l1_cache[query_index]==query_tag_l1) // true if line of l1 cache contains required address
			{
				...
				cout<<"Value is : "<<l1_cache[query_index][query_offset]<<endl;
			}
	...
	fp.close();
 ll net_hits=number_of_hits_l1+number_of_hits_victim;
 ll net_misses=number_of_misses_victim;
...
cout<<"net hits : "<<net_hits<<endl;
cout<<"net misses : "<<net_misses<<endl;
cout<<"Improvement in hits(%) : "<<100.0*((double)(net_hits-number_of_hits_l1)/(double)number_of_hits_l1)<<"%"<<endl;
